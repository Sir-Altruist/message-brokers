...
class RabbitMQ {
    ...
    async connect(){
        ...
        try {
            console.log('Connecting to RabbitMQ consumer one server...')
            this.#connection = await amqplib.connect('amqp://localhost')
            console.log('RabbitMQ consumer one server connection is ready...')
            this.#channel = await this.#connection.createChannel()
            console.log('RabbitMQ consumer one channel was created successfully...')
        } catch (error) {
            throw error
        }
    }
    ...
    async consumeMessage(){
        ...
        /** This consumes messages from queue */
        this.#channel.consume(queue.queue, (msg) => {
            if(msg !== null){
                console.log(`Consumer one received ${msg.content.toString()} from publisher`)
            }
