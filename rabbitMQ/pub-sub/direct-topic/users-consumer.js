...
class RabbitMQ {
    ...
    async connect(){
        ...
        try {
            console.log('Connecting to RabbitMQ users consumer server...')
            this.#connection = await amqplib.connect('amqp://localhost')
            console.log('RabbitMQ user consumer server connection is ready...')
            this.#channel = await this.#connection.createChannel()
            console.log('RabbitMQ users consumer channel was created successfully...')
        } catch (error) {
            throw error
        }
    }
    ...
    async consumeMessage(){
        ...
        const routingKey = 'users.#' // any key beginning with users will be routed here
        ...
        /** This consumes from the queue */
        this.#channel.consume(queue.queue, (msg) => {
            if(msg !== null){
                console.log(`Users service received ${msg.content.toString()} from publisher`)
            }
