...
class RabbitMQ {
    ...
    async connect(){
        ...
        try {
            console.log('Connecting to RabbitMQ analytics consumer server...')
            this.#connection = await amqplib.connect('amqp://localhost')
            console.log('RabbitMQ analytics consumer server connection is ready...')
            this.#channel = await this.#connection.createChannel()
            console.log('RabbitMQ analytics consumer channel was created successfully...')
        } catch (error) {
            throw error
        }
    }
    ...
    async consumeMessage(){
        ...
        const routingKey = '*.europe.*' // any key having europe between it (with exactly one word before and after) will be routed here
         ...
         * 'exclusive:true' only allows on consumer at a time. Can only be consume on the same connection channel it was set on
         **/
